...
export class GetOneSchedule {
  ...
  constructor(
    ...
    this.options = {
      url: `/schedule/${JSON.parse(localStorage.getItem('currentUser')).practitionerId}`,
      handlers: {
  ...
  public success(user: GetCredentialsResponse): void {
    this.bus.publish(this.events.received.schedule.one.success, user);
  }
  ...
  public subscribe(): void {
    this.bus.subscribe(this.events.requested.data.schedule.one, this.request.bind(this));
  }
}