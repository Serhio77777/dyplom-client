...
export class EditPatient {
  ...
  public success(user: PutCredentialsResponse): void {
    // TODO: patient
    this.location.back();
    this.bus.publish(this.events.received.patients.edit.success, user);
  }
  ...
  private request(data): void {
    ...
    this.options.body.patientDateBorn = format(this.options.body.patientDateBorn, 'YYYY-MM-DD');

    this.requestService.put<PutCredentialsRequest, PutCredentialsResponse>(this.options);
  }

  public subscribe(): void {
    this.bus.subscribe(this.events.requested.data.patients.edit, this.request.bind(this));
  }
}