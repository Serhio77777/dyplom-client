...
export class GetAllPatients {
  ...
  constructor(
    ...
    this.options = {
      url: `/patients/${JSON.parse(localStorage.getItem('currentUser')).practitionerId}`,
      handlers: {
  ...
  public success(user: GetCredentialsResponse): void {
    this.bus.publish(this.events.received.patients.all.success, user);
  }
  ...
  public subscribe(): void {
    this.bus.subscribe(this.events.requested.data.patients.all, this.request.bind(this));
  }
}